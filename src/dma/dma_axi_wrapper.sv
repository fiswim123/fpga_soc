...
#(
  parameter int DMA_ID_VAL = 8'h20
)(
  input                 clk,
  input                 rst,
  // CSR DMA I/F
  ...
  // 注意：dma_csr_miso_o.rdata 现在完全由 csr_dma 驱动，不再在 always_comb 中赋值
  always_comb begin
    ...
    //end
    // 将32位 CSR 数据扩展到 AXI 数据宽度（高位补0）
    //assign dma_csr_miso_o.rdata = {{(AXI_DATA_WIDTH-32){1'b0}}, csr_rdata};
  ...
  dma_csr #(
    ...
    .ADDRESS_WIDTH                  (`AXI_ADDR_WIDTH),  // 新增
    .DATA_WIDTH                     (32)    // 固定为32位（AXI-Lite 标准数据宽度）
  ) u_dma_csr (
    .i_clk                          (clk),
    .i_rst_n                        (~rst),
    ...
    .o_rdata                        (dma_csr_miso_o.rdata),// 直接连接，宽度匹配
