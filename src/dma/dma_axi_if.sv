...
#(
  ...
  // === [A] 新增寄存器定义（放在现有 reg/logic 定义区）===
logic        wburst_active_ff, next_wburst_active;
axi_alen_t   wburst_alen_ff,   next_wburst_alen;
axi_wr_strb_t wburst_strb_ff,  next_wburst_strb;
...
always @(posedge clk) begin
    if (dma_mosi_o.arvalid && dma_miso_i.arready) begin
        $display("[DMA_IF] AR handshake: addr=%h, len=%0d, size=%0d", 
                 dma_mosi_o.araddr, dma_mosi_o.arlen, dma_mosi_o.arsize);
    end
end
...
always @(posedge clk) begin
    if (dma_mosi_o.awvalid && dma_miso_i.awready) begin
        $display("[DMA_IF] AW handshake: addr=%h, len=%0d, size=%0d", 
...
always_ff @(posedge clk) begin
  if (dma_mosi_o.awvalid && dma_miso_i.awready) begin
    $display("[DMA_IF] AW handshake at %t: addr=%h, len=%0d", $time, dma_mosi_o.awaddr, dma_mosi_o.awlen);
  end
end
always_ff @(posedge clk) begin
  if (dma_mosi_o.wvalid && dma_miso_i.wready) begin
    $display("[DMA_IF] W beat at %t: beat=%0d, last=%b, data=%h, strb=%b", 
             $time, beat_counter_ff, dma_mosi_o.wlast, dma_mosi_o.wdata, dma_mosi_o.wstrb);
    if (dma_mosi_o.wlast)
      $display("[DMA_IF] WLAST beat");
  end
end
always_ff @(posedge clk) begin
  if (dma_miso_i.bvalid && dma_mosi_o.bready) begin
    $display("[DMA_IF] B response at %t: resp=%0d", $time, dma_miso_i.bresp);
  end
end
always @(posedge clk) begin
  if (dma_mosi_o.arvalid && dma_miso_i.arready) begin
    $display("[DMA_IF] AR handshake at %t: addr=%h, len=%0d", $time, dma_mosi_o.araddr, dma_mosi_o.arlen);
  end
end
always @(posedge clk) begin
  if (dma_miso_i.rvalid && dma_mosi_o.rready) begin
    $display("[DMA_IF] R beat at %t: last=%b, data=%h", $time, dma_miso_i.rlast, dma_miso_i.rdata);
  end
end
always @(posedge clk) begin
  if (dma_axi_rd_req_i.valid) begin
    $display("[DMA_IF] Read request valid: addr=%h, alen=%0d", dma_axi_rd_req_i.addr, dma_axi_rd_req_i.alen);
  end
end
always @(posedge clk) begin
  if (dma_fifo_req_o.rd) $display("[DMA_IF] Write FIFO read at %t", $time);
  if (dma_fifo_req_o.wr) $display("[DMA_IF] Write FIFO write at %t, data=%h", $time, dma_fifo_req_o.data_wr);
end
always @(posedge clk) begin
  if (dma_axi_rd_req_i.valid && dma_axi_rd_resp_o.ready) begin
    $display("[DMA_IF] Read request accepted by AXI interface");
  end
  if (dma_axi_wr_req_i.valid && dma_axi_wr_resp_o.ready) begin
    $display("[DMA_IF] Write request accepted by AXI interface");
  end
end
always_ff @(posedge clk) begin
  if (wr_data_txn_hpn) begin
    $display("[DMA_IF] Pop write request (burst complete)");
  end
end
always_ff @(posedge clk) begin
  if (dma_fifo_resp_i.empty) begin
    $display("[DMA_IF] data FIFO empty at %t, beat=%0d, wvalid=%b", $time, beat_counter_ff, dma_mosi_o.wvalid);
  end
end
*/
  always_comb begin
...
if (dma_active_i) begin
  ...
  if (wr_txn_hpn && !wr_resp_hpn) begin
    ...
    //$display("[DMA_IF] B response decrementing wr_counter from %0d to %0d", wr_counter_ff, wr_counter_ff - 1);
  end
end else begin
  next_rd_counter = '0;
  next_wr_counter = '0;
...
if (dma_active_i &&
    !dma_axi_rd_req_i.valid &&
    !dma_axi_wr_req_i.valid &&
    !dma_miso_i.rvalid &&
    !dma_miso_i.bvalid &&
    !aw_txn_started_ff &&
    (wr_counter_ff == 0) &&
    (rd_counter_ff == 0) &&
    (beat_counter_ff == 0)) begin
  next_rd_counter = '0;
  next_wr_counter = '0;
end
    ...
    // page 44 - AXI4 Spec)
    // 替换原有的 wr_new_txn 生成部分

    if (wr_new_txn) begin
  //$display("[DMA_IF] Push write request: alen=%0d, strb=%b", wr_data_req_in.alen, wr_data_req_in.wstrb);
end
...
always_comb begin
  ...
  always_comb begin : axi4_master
    ...
    if (dma_active_i) begin
      ...
      // AW 只由写请求和outstanding控制，不要被数据FIFO empty门控
