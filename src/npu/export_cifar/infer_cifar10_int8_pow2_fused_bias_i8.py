...
def requantize(acc, bias, shift, relu, bias_domain):
    if bias_domain == "int8_output":
        out = apply_shift(acc.astype(np.int64), int(shift)) + bias.astype(np.int64)
    elif bias_domain == "int32_accumulator":
        out = apply_shift(acc.astype(np.int64) + bias.astype(np.int64), int(shift))
    else:
        raise ValueError(f"Unsupported bias_domain: {bias_domain}")
...
def conv2d_int8(x, layer, pad=2, stride=1):
    n = x.shape[0]
    weight = layer["weight"]
...
def resolve_manifest_path(asset_dir, manifest_name):
    path = os.path.join(asset_dir, manifest_name)
    if os.path.exists(path):
        return path
    fallback = os.path.join(asset_dir, "manifest.json")
    if os.path.exists(fallback):
        return fallback
    raise FileNotFoundError(f"Neither {path} nor {fallback} exists")


def resolve_source_asset_dir(asset_dir, manifest):
    source = manifest.get("source_asset_dir")
    if source:
        source_path = source if os.path.isabs(source) else os.path.join(asset_dir, source)
        return os.path.abspath(source_path)
    return os.path.abspath(asset_dir)


def resolve_asset_file(asset_dir, source_asset_dir, file_name):
    local_path = os.path.join(asset_dir, file_name)
    if os.path.exists(local_path):
        return local_path
    source_path = os.path.join(source_asset_dir, file_name)
    if os.path.exists(source_path):
        return source_path
    raise FileNotFoundError(f"Cannot find {file_name} in {asset_dir} or {source_asset_dir}")


def load_assets(asset_dir, manifest_name):
    manifest_path = resolve_manifest_path(asset_dir, manifest_name)
    manifest = load_json(manifest_path)
    source_asset_dir = resolve_source_asset_dir(asset_dir, manifest)

    layers = {}
    for layer in manifest["layers"]:
        name = layer["name"]
        bias_domain = layer.get("bias_domain", "int32_accumulator")
        bias_bits = 8 if bias_domain == "int8_output" else 32
        layers[name] = {
            "weight": read_memh(
                resolve_asset_file(asset_dir, source_asset_dir, layer["weight_file"]),
                bits=8,
                shape=layer["weight_shape"],
            ).astype(np.int32),
            "bias": read_memh(
                resolve_asset_file(asset_dir, source_asset_dir, layer["bias_file"]),
                bits=bias_bits,
...
def infer_batch(images_q, layers):
    ...
    x = conv2d_int8(x, layers["conv1"])
    x = maxpool2x2(x)
    x = conv2d_int8(x, layers["conv2"])
    x = maxpool2x2(x)
    x = avgpool8x8_shift(x)
    logits = linear_int8(x, layers["fc"])
    return logits.astype(np.int32)


def load_image_manifest(image_int8_dir):
    return load_json(os.path.join(image_int8_dir, "manifest.json"))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run TinyCIFAR10_5x5 inference with derived INT8 output-domain bias assets."
    )
    parser.add_argument("--asset-dir", default="./cifar10_int8_pow2_fused_bias_i8")
    parser.add_argument("--manifest-name", default="manifest_bias_i8.json")
...
def main():
    args = parse_args()
    _manifest, layers = load_assets(args.asset_dir, args.manifest_name)
    ...
    if labeled:
        ...
        print(f"INT8 pow2 fused bias-i8 accuracy from INT8 images: {acc:.3f}% ({correct}/{seen})")
    else:
        print(f"Ran INT8 pow2 fused bias-i8 inference on {seen} INT8 image(s).")
