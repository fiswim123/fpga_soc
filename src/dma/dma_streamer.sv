...
#(
  ...
  function automatic axi_alen_t great_alen(axi_addr_t addr, desc_num_t bytes);
    ...
    // 函数体开始
    hw_max_beats = `DMA_MAX_BEAT_BURST;
    bytes_per_beat = `DMA_DATA_WIDTH / 8;
    cfg_max_beats = dma_maxb_i + 1;
...
/*
  function automatic axi_alen_t great_alen(axi_addr_t addr, desc_num_t bytes);
    int max_beats;
    int hw_max_beats = `DMA_MAX_BEAT_BURST;          // 硬件支持的最大 beat 数（如 256）
    int cfg_max_beats = dma_maxb_i + 1;              // 软件配置的最大 beat 数（max_burst+1）
    int bytes_per_beat;
    int max_by_bytes;
    // 取硬件和软件限制的较小值
    max_beats = (cfg_max_beats < hw_max_beats) ? cfg_max_beats : hw_max_beats;
    // 限制不超过剩余字节数所能传输的 beat 数
    bytes_per_beat = `DMA_DATA_WIDTH / 8;
    max_by_bytes = bytes / bytes_per_beat;
    if (max_by_bytes < max_beats) max_beats = max_by_bytes;
    // 至少传输 1 个 beat
    if (max_beats < 1) max_beats = 1;
    // 返回 AXI len 值（beat 数 - 1），并确保不超出 0~255
    return (max_beats - 1);
endfunction
*/
/*
  function automatic axi_alen_t great_alen(axi_addr_t addr, desc_num_t bytes);
    ...
    desc_num_t txn_sz;
    // 保留原有 great_alen 打印
    //$display("great_alen: addr=0x%08x, bytes=%0d, maxb=%0d", addr, bytes, dma_maxb_i);
    for (int i=`DMA_MAX_BEAT_BURST; i>0; i--) begin
        ...
        if ((bytes >= txn_sz) && ((`DMA_MAX_BURST_EN == 1) ? ((i-'d1) <= dma_maxb_i) : 1'b1) && ((`DMA_MAX_BEAT_BURST > 16) ? valid_burst(dma_mode_ff, i[8:0]) : 1'b1)) begin
            if (burst_r4KB(addr, fut_addr)) begin
                alen = axi_alen_t'(i-1);
                //$display("great_alen returns: i=%0d, alen=%0d", i, alen);
                return alen;
            end
        end
    end
    //$display("great_alen returns 0 (no suitable burst)");
    return alen;
  endfunction
*/
  ...
  always_comb begin : burst_calc
    ...
    if ((cur_st_ff == DMA_ST_SM_IDLE) && (next_st == DMA_ST_SM_RUN)) begin
      ...
      // 添加打印：描述符初始化
      /*$display("Streamer %s: initialized, idx=%0d, addr=0x%08x, bytes=%0d, mode=%s",
               STREAM_TYPE ? "WRITE" : "READ",
               dma_stream_i.idx,
               next_desc_addr,
               next_desc_bytes,
               (next_dma_mode == DMA_MODE_FIXED) ? "FIXED" : "INCR");*/
    end
...
/*
          // 添加打印
          $display("[STREAMER] %s burst: addr=0x%08x, alen=%0d, size=%0d, strb=0x%0x, bytes_rem=%0d",
             STREAM_TYPE ? "WRITE" : "READ",
             next_dma_req.addr,
             next_dma_req.alen,
             next_dma_req.size,
